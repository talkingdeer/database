	...
	try{
		...
		if(*it == "?"){
			if(++it != commands.end()){
				...
				answer = "List of commands: <br>1. Print<br>2. Generate [number of people]<br>3. Save<br>4. Load<br>5. Add [first name, last name, surname, date of birth, citizenship, exit permit, rating]<br>6. Remove [last name]<br>7. Find lastname [name] || exitpermit [exitpermit] || rating [< or > smth] || date [date] (or multiple options using && between them)<br>8. Size\n";
				return answer;
			}
		} else if (*it == "Print"){
			if(++it != commands.end()){
			...
			if(flag){
				return add(base, lastname, firstname, surname, dateofbirth, citizenship, exitpermit, rating);
			}
		} else if (*it == "Find"){
			if(++it == commands.end()){
				flag = false;
				...
				if(flag){
					return find(i, base, lastname, is_lastname, rating_greater, rating_less, is_rating, is_greater, is_less, exitPermit, is_exitPermit, date, is_date);
				}
			}
		} else if (*it == "Remove"){
			if(++it == commands.end()){
...
string print(int i, list<Person>& base)
{
	...
	if(base.empty()) {
		...
		for(auto it = base.begin(); it != base.end(); it++){
			answer = answer + person_to_str(*it) + "<br>";
		}
	}
	return answer;
}
